...
class modelEqns():
    ...
    def __init__(self):
        ...
        self._res0             = np.zeros(num_times+1)
        self._res1             = np.zeros(num_times+1)
        self._res2             = np.zeros(num_times+1)
        self._res3             = np.zeros(num_times+1)
...
##### Initial Conditions #####
        ...
        self._res0[1]    = self.params._res00
        self._res1[1]    = self.params._res10
        self._res2[1]    = self.params._res20
        self._res3[1]    = self.params._res30
    ...
    def simulateDynamics(self,x):
        ...
        for i in range(2, self.params._num_times+1):
            ...
            self._res0[i] = (self.params._emshare0 * self.params._tau0 * self._alpha[i] * 
                                    (self._eco2[i] / 3.667) * 
                                    (1 - math.exp(-self.params._tstep / (self.params._tau0 * self._alpha[i]))) + 
                                    self._res0[i-1] * math.exp(-self.params._tstep / (self.params._tau0 * self._alpha[i])))

            self._res1[i] = (self.params._emshare1 * self.params._tau1 * self._alpha[i] * 
                                    (self._eco2[i] / 3.667) * 
                                    (1 - math.exp(-self.params._tstep / (self.params._tau1 * self._alpha[i]))) + 
                                    self._res1[i-1] * math.exp(-self.params._tstep / (self.params._tau1 * self._alpha[i])))

            self._res2[i] = (self.params._emshare2 * self.params._tau2 * self._alpha[i] * 
                                    (self._eco2[i] / 3.667) * 
                                    (1 - math.exp(-self.params._tstep / (self.params._tau2 * self._alpha[i]))) + 
                                    self._res2[i-1] * math.exp(-self.params._tstep / (self.params._tau2 * self._alpha[i])))

            self._res3[i] = (self.params._emshare3 * self.params._tau3 * self._alpha[i] * 
                                    (self._eco2[i] / 3.667) * 
